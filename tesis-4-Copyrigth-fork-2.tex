...
\end{equation} where the terms $s_t$ and $b_t$ are the level and slope (trend) at time $t$. This model is used to predict stable long-term time series. The $h$-step ahead prediction, also named Holt's method, is performed by Equation \ref{holtsmethod}

\begin{equation}\label{holtsmethod}
	\hat{Y}_{t+h}=s_{t-1} +h b_{t-1}
...
s_{t-1} &=& Y_{t-1} + (1-\alpha)^2 \epsilon_{t-1} \\
...
b_t &=& \alpha_2 (s_t-s_{t-1})+ (1-\alpha_2)b_{t-1} \label{errorhwmode-1}
\end{eqnarray}
