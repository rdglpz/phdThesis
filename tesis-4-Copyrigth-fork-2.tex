...
{\mathbf{Y}}_{T \tau+1} = \{Y_{\tau (T-1)+1},\dots,Y_{\tau T \cdot  h}  \}
\end{equation}

\noindent where $h$ is the $h$-th predicted element. The Figure \ref{predictionSample} shows how the prediction at the smallest basis (hours) is performed along the period (day), using a sliding window of size $h$ covering the prediction subsequence data from $(\tau (T-1)+1)$ to $(\tau T \cdot  h)$ in $\tau$ steps. 
...
\begin{equation}\label{corrector}
	\{ \mathbf{\hat{Y}}_{T \tau+i}\}_{i=1}^{H \tau}   =  \{ \mathbf{\hat{Z}}_{T+i} \cdot \mathbf{{P}}_{\hat{C}_{T+i}}+\alpha F_{T+i} \}_{i=1}^{H}
